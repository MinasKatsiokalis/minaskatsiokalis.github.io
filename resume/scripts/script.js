document.addEventListener('DOMContentLoaded', function() {
  ...
  // Job item expansion: hover on desktop, click on mobile
  ...
  jobItems.forEach(item => 
  {
    ...
    if (isMobile) 
    {
      // On mobile: expand/collapse on click
      item.addEventListener('click', function(e) 
      {
        // Prevent toggling if clicking inside the details content
        if (e.target.closest('.job-details-content')) 
          return;
        const expanded = detailsContent.style.maxHeight && detailsContent.style.maxHeight !== '0px';
        if (expanded) 
        {
